...
def _extract_primary_outcome(evaluation: TradingEvaluation) -> TradingOutcome | None:
    if evaluation.outcomes and len(evaluation.outcomes) > 0:
...
def compute_kpis(evaluations: list[TradingEvaluation]) -> AnalyticsKpiPayload:
    ...
    for evaluation in evaluations:
        outcome = _extract_primary_outcome(evaluation)
        if outcome is None:
            continue
...
def compute_pnl_drivers_heatmap(evaluations: list[TradingEvaluation]) -> list[AnalyticsHeatmapSeriesPayload]:
    closed_evaluations = [evaluation for evaluation in evaluations if _extract_primary_outcome(evaluation) is not None]
