...
_cached_solana_gas_budget_snapshot: TradingWalletMaintenanceSolanaGasBudgetSnapshot | None = None
_budget_snapshot_cache_lock = threading.Lock()


def clear_solana_gas_budget_snapshot_cache() -> None:
    global _cached_solana_gas_budget_snapshot
    _cached_solana_gas_budget_snapshot = None
...
def build_solana_gas_budget_snapshot(
    ...
    global _cached_solana_gas_budget_snapshot
    if not force_refresh and _cached_solana_gas_budget_snapshot is not None:
        return _cached_solana_gas_budget_snapshot

    with _budget_snapshot_cache_lock:
        if not force_refresh and _cached_solana_gas_budget_snapshot is not None:
            return _cached_solana_gas_budget_snapshot

        cost_snapshot = build_solana_gas_reserve_cost_snapshot(force_refresh=force_refresh)
        minimum_cycle_number = settings.TRADING_GAS_MINIMUM_CYCLE_NUMBER
        refill_target_cycle_number = settings.TRADING_GAS_REFILL_TARGET_CYCLE_NUMBER
        ...
        _cached_solana_gas_budget_snapshot = TradingWalletMaintenanceSolanaGasBudgetSnapshot(
