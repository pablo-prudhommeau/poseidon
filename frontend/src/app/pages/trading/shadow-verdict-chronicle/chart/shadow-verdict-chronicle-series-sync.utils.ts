...
function synchronizeChronicleVerdictCloud(
...
function synchronizeChronicleTapeBoundMetrics(
    model: ChronicleChartModel,
    arrays: ChronicleArrays,
    ...
    for (let index = 0; index < model.movingAverageLineRenderableSeries.length; index++) {
        ...
        synchronizeXySeries(
            lineDataSeries,
            arrays.metricTimestampsMilliseconds,
            movingAverageSeriesValues[index] ?? [],
        );
    }
}

export function synchronizeChronicleSeriesFromArrays(
    model: ChronicleChartModel,
    arrays: ChronicleArrays,
    thresholds: ChronicleGoldenZoneThresholds,
): void {
...
export function synchronizeChronicleTapeBoundSeries(
    model: ChronicleChartModel,
    arrays: ChronicleArrays,
    thresholds: ChronicleGoldenZoneThresholds,
): void {
...
export function synchronizeChronicleVerdictCloudSeries(
    model: ChronicleChartModel,
    arrays: ChronicleArrays,
): void {
    synchronizeChronicleVerdictCloud(model, arrays);
