...
import type {ChronicleAdjustTooltipPositionHost, ChronicleArrays, ChronicleBucketMeta, ChronicleChartModel, ChronicleThresholdPaletteBinding, SciChartModule,} from '../data/shadow-verdict-chronicle.models';
...
function createSmaThresholdPaletteController(
    sci: SciChartModule,
...
export function buildChronicleSeriesBundle(
    ...
    const goldenZoneExpectedValueBandDataSeries = new XyDataSeries(wasmContext, {
        ...
        yValues: expectedValueThreshold == null
            ? chronicleArrays.movingAverageExpectedValueSeries.map(() => 0)
            : chronicleArrays.movingAverageExpectedValueSeries.map((value: number) =>
                value > expectedValueThreshold ? value : expectedValueThreshold,
            ),
    ...
    const goldenZoneProfitFactorBandDataSeries = new XyDataSeries(wasmContext, {
        xValues: chronicleArrays.metricTimestampsMilliseconds,
        yValues: profitFactorThreshold == null
            ? chronicleArrays.movingAverageProfitFactorSeries.map(() => 0)
            : chronicleArrays.movingAverageProfitFactorSeries.map((value: number) =>
                value > profitFactorThreshold ? value : profitFactorThreshold,
            ),
    ...
    const goldenZoneExpectedValueSmaPaletteController = createSmaThresholdPaletteController(
        sci,
        '#A7F3D0',
        '#3F8F75',
    );
    const goldenZoneProfitFactorSmaPaletteController = createSmaThresholdPaletteController(
        sci,
        '#FDE68A',
        '#9B7A22',
    );
    ...
    movingAverageExpectedValueLineSeries.paletteProvider =
        goldenZoneExpectedValueSmaPaletteController.paletteProvider as never;
    movingAverageProfitFactorLineSeries.paletteProvider =
        goldenZoneProfitFactorSmaPaletteController.paletteProvider as never;
    ...
    profitableVerdictBubbleSeries.runAnimation(
        new BubbleAnimation({...sweep, dataSeries: profitableVerdictXyzDataSeries}),
    );
    lossVerdictBubbleSeries.runAnimation(
        new BubbleAnimation({...sweep, dataSeries: lossVerdictXyzDataSeries}),
    );
    ...
    sciChartSurface.chartModifiers.add(
        ...
        new CursorModifier({
            ...
            tooltipSvgTemplate: (seriesInfos, svgAnnotation) =>
                buildChronicleCursorTooltipSvg(
                    sci as unknown as ChronicleAdjustTooltipPositionHost,
                    seriesInfos as never,
                    svgAnnotation,
                ),
        }),
        new LegendModifier({
