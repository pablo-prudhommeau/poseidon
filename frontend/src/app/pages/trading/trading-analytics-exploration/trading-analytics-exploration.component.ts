...
@Component({
    selector: 'app-trading-analytics-exploration',
    standalone: true,
    imports: [CommonModule, NgApexchartsModule],
    templateUrl: 'trading-analytics-exploration.component.html',
    styleUrl: 'trading-analytics-exploration.component.css'
})
export class TradingAnalyticsExplorationComponent implements OnChanges {
    @Input() heatmapData: TradingAnalyticsHeatmapSeriesPayload[] = [];
    ...
    private rebuildCharts(): void {
        ...
        if (tab === 'non-linear' || tab === 'outlier') {
            ...
            const sortedSeries = category.metricKeys
                .map((key) => this.heatmapData.find((series) => series.metric_key === key))
                .filter((series): series is TradingAnalyticsHeatmapSeriesPayload => !!series);
                ...
                if (tab === 'non-linear') {
                    ...
                    newConfigs.push({
                        ...
                        markers: {
                            size: 4,
                            strokeWidth: 0,
                            colors: ['transparent', POSEIDON_COLORS.success, '#a78bfa', '#f87171']
                        },
                        xaxis: {
                        ...
                        yaxis: [
                            ...
                            },
                            {
                                ...
                                labels: {
                                    offsetX: -10,
                                    style: { colors: POSEIDON_COLORS.success, fontSize: '9px' },
                                    formatter: (val: number) => `${Number(val).toFixed(0)}% WR`
                                },
                                title: { text: undefined },
                                min: 0,
                                max: 100
                            },
                            {
                                ...
                                labels: {
                                    ...
                                    formatter: (val: number) => (val ? `${Number(val).toFixed(1)} vel` : '0.0 vel')
                                },
                                title: { text: undefined }
                            },
                            {
                                ...
                                labels: {
                                    offsetX: -10,
                                    style: { colors: '#f87171', fontSize: '9px' },
                                    formatter: (val: number) => (val ? `${Number(val).toFixed(0)}% PnL` : '0% PnL')
                                },
                                title: { text: undefined }
                            }
                        ]
                    ...
                    newConfigs.push({
                        ...
                        yaxis: [
                            ...
                            },
                            {
                                ...
                                labels: {
                                    offsetX: -10,
                                    style: { colors: '#fbbf24', fontSize: '9px' },
                                    formatter: (val: number) => `${Number(val).toFixed(0)}% HR`
                                },
                                title: { text: undefined }
                            }
                        ]
            ...
            const sortedSeries = category.metricKeys
                .map((key) => this.scatterData.find((series) => series.metric_key === key))
                .filter((series): series is TradingAnalyticsScatterSeriesPayload => !!series);

            for (const series of sortedSeries) {
                const scatterPoints = series.points.map((point) => [point.metric_value, point.pnl_percentage]);

                newConfigs.push({
                    metricLabel: series.metric_label,
                    series: [{ name: 'Trades', type: 'scatter', data: scatterPoints as never[] }],
                    chart: {
                        type: 'scatter',
                        height: 250,
                        parentHeightOffset: 0,
