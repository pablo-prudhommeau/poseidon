...
import { TradingAnalyticsKpiPayload, TradingAnalyticsTimelinePointPayload } from '../../../core/models';

@Component({
    selector: 'app-trading-analytics-kpi-bar',
    standalone: true,
    imports: [CommonModule, NgApexchartsModule],
    templateUrl: 'trading-analytics-kpi-bar.component.html',
    styleUrl: 'trading-analytics-kpi-bar.component.css'
})
export class TradingAnalyticsKpiBarComponent implements OnChanges {
    @Input() kpis: TradingAnalyticsKpiPayload | null = null;
    @Input() timeline: TradingAnalyticsTimelinePointPayload[] = [];
    ...
    private buildSparkline(): void {
        const data = this.timeline.map((timelinePoint) => timelinePoint.cumulative_pnl_usd);
